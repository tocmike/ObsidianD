...
### 儿童蜡笔画
Nano Banana Pro 提示：像一个开朗的6岁孩子在白色笔记本纸上创作的画面一样绘制[视觉]。使用粗糙的蜡笔笔触、摇晃的轮廓和简单的平面形状。颜色应当明亮、粗犷，略微超出边界，带有明显的压力痕迹和小污迹。透视是无害的：物体可以漂浮，尺寸基于重要性而非真实性。脸部是带着弧度微笑的点状;手是连指手套状的团块;动物是圆润且友好的。添加有趣的额外元素，比如角落里的太阳、蓬松的云朵、星星和爱心。可以加上不均匀的涂鸦、边缘几处小涂鸦，以及一两个用孩子般拼写的手写标签。 所有提示都用ALT格式表示图片。
